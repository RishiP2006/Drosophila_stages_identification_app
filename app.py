        ...
        if self.model is not None:
            if self.info.get("type") == "classification":
                ...
                for name, conf, (x1, y1, x2, y2) in dets:
                    ...
                    draw.text((x1, max(y1-10, 0)), f"{name} {conf:.2f}", fill="green", font=font)
...
st.markdown("---")
st.write("**Notes:**")
st.write(f"- Models from Hugging Face: `{HF_REPO_ID}`")
st.write("- Classification assumes binary output.")
st.write("- YOLO models output bounding boxes with class names.")
st.write("- Live camera uses PIL for drawing, no OpenCV needed.")
