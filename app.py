import os
os.environ["TF_USE_LEGACY_KERAS"] = "1"   # <<-- critical line
...
with st.sidebar:
    ...
    # prefer your ResNet name if present
...
class LiveVideoProcessor(VideoProcessorBase):
    ...
    def _predict(self, bgr: np.ndarray):
        ...
        img = cv2.resize(img, self.bundle.input_hw[::-1])
        ...
        probs = softmax(preds) if (np.max(preds) > 1.0 or np.min(preds) < 0.0) else preds
    ...
    def recv(self, frame: av.VideoFrame) -> av.VideoFrame:
        ...
        self._frame_count += 1
        if (self._frame_count % self.n_skip == 0) or (self._last_pred is None):
            try:
